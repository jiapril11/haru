...
export default function TodoItem({ todo }: Props) {
  ...
  return (
    <div className="group flex items-center gap-3 rounded-xl border border-white/10 bg-[#16213e] px-4 py-3">
      <button
        onClick={() =>
      ...
      <span
        className={`flex-1 text-sm transition-colors ${
          todo.is_done ? "text-white/30 line-through" : "text-white"
        }`}
      >
        {todo.title}
      </span>
      ...
      {/* 우선순위 */}
      <span
        className={`rounded-full px-2 py-0.5 text-xs ${priorityStyle[todo.priority]}`}
      >
