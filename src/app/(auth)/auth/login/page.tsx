...
export default function LoginPage() {
  ...
  function incrementResetCount() {
    ...
    if (!firstAt || now - firstAt > RESET_WINDOW) {
      localStorage.setItem("reset_attempts", JSON.stringify({ count: 1, firstAt: now }));
    } else {
      localStorage.setItem("reset_attempts", JSON.stringify({ count: count + 1, firstAt }));
    }
  }
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    if (mode === "forgot") {
      ...
      if (isWithinWindow && count >= RESET_LIMIT) {
        setError(`재설정 요청은 1시간에 ${RESET_LIMIT}번까지만 가능합니다. ${getRemainingTime(firstAt)} 후에 다시 시도해주세요.`);
        setLoading(false);
        return;
      }
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8">
        ...
        {mode === "forgot" && (
          <div className="mb-6">
            ...
            <p className="mt-3 text-sm font-medium text-[var(--text)]">비밀번호 찾기</p>
            <p className="mt-1 text-xs text-[var(--text-subtle)]">가입한 이메일로 재설정 링크를 보내드려요.</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {mode === "login" && (
            <button
              ...
              className="text-center text-xs text-[var(--text-faint)] hover:text-[var(--text-subtle)]"
            >
              비밀번호를 잊으셨나요?
            </button>
          )}
